...
static uint8_t felica_idm[]={0x01,0x12,0x03,0x12,0xEC,0x1F,0x88,0x00};
static uint8_t felica_pmm[]={0x05,0x31,0x43,0x45,0x46,0x82,0xB7,0xFF};
...
void setup(){
    ...
    felica=new Felica(felica_pmm);
    felica->add_system(0x0003,felica_idm);
    FelicaSystem *system;
    felica->get_system(0x0003,&system);
    system->add_service(0x090F);
    FelicaService *service;
    system->get_service(0x090F,&service);
    for(uint16_t i=0;i<5;i++){
        const uint8_t year=4;
        const uint8_t month=12;
        const uint8_t day=1+i;
        block_data[i][4]=(year<<1)+((month&0b1000)>>3);
        block_data[i][5]=((month&0b111)<<5)+(day&0b11111);
    ...
    system->add_service(0x008B);
    system->get_service(0x008B,&service);
    service->write_block_force(0,block_data[19]);

    system->add_service(0x108F);
    system->get_service(0x108F,&service);
    service->write_block_force(0,block_data[18]);
}
